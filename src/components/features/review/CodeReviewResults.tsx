...
const CodeReviewResultsComponent = forwardRef<CodeReviewResultsRef, CodeReviewResultsProps>(
  ...
  const getAnalyzingMessage = (type: ReviewType) => {
    const baseMessages = {
      ...
      encouraging: "Lagi review kode keren lu dengan teliti... Hampir selesai! 🌟",
      codeQuality: "Lagi analisis code quality secara comprehensive... Tunggu ya ⚙️",
    ...
    if (isRetrying && retryCount > 0) {
      const retryMessages: Record<ReviewType, string> = {
        sarcastic: `${baseMessage} (Percobaan ${retryCount + 1} - Pantang menyerah! 😤)`,
        brutal: `${baseMessage} (RETRY ${retryCount + 1} - JANGAN PERNAH NYERAH! 💪)`,
        encouraging: `${baseMessage} (Coba ${retryCount + 1} - Pasti bisa! 🌈)`,
        codeQuality: `${baseMessage} (Percobaan ${retryCount + 1})`,
        security: `${baseMessage} (Retry ${retryCount + 1} - Keamanan penting! 🛡️)`,
        bestPractices: `${baseMessage} (Percobaan ${retryCount + 1} - Excellence butuh kesabaran! ⭐)`
      };
  ...
  const getUploadedMessage = (type: ReviewType) => {
    const messages = {
      sarcastic: "File udah keupload. Mari kita liat apa yang mau dianalisis... 🧐",
      ...
      bestPractices: "File udah keupload. Review best practices dimulai... 📖"
    };
    return messages[type] || messages.sarcastic;
  };
...
function checkBestPractices() {
  ...
  const getTipMessage = (type: ReviewType) => {
    const tips = {
      sarcastic: "💡 Pro tip: Makin questionable kode lu, makin seru review-nya! Sekarang dengan roasting yang lebih PANJANG dan DETAIL! Upload apa aja - gue ga judge kok... banyak 🤭",
      brutal: "⚠️ Peringatan: Reviewer ini GA ADA AMPUN dengan analisis yang COMPREHENSIVE dan BRUTAL! Upload kode cuma kalo lu sanggup nerima KEBENARAN DETAIL yang panjang lebar! 💀",
      encouraging: "✨ Inget: Setiap kesalahan adalah kesempatan belajar! Sekarang dengan feedback yang lebih MENDALAM dan SUPPORTIVE! Upload kode lu dan mari berkembang bareng! 🌱",
      codeQuality: "📊 Enhanced: Sekarang dengan analisis yang lebih komprehensif, detailed metrics, dan rekomendasi yang lebih mendalam untuk hasil review yang maksimal.",
      security: "🔒 Advanced Security: Scanning yang lebih detail dengan risk assessment, attack vectors, dan mitigation strategies yang comprehensive untuk keamanan maksimal.",
      bestPractices: "📚 Comprehensive: Evaluasi mendalam terhadap design patterns, quality metrics, dan industry standards dengan rekomendasi yang lebih actionable."
    };
                ...
                <>
                  {reviewResults.metadata.overallAssessment && (
                    <div className="bg-sky/20 border-2 border-charcoal rounded-lg p-3">
                      <h3 className="font-bold text-charcoal text-sm mb-2">📊 Overall Assessment</h3>
                  ...
                  {reviewResults.metadata.overallSecurityAssessment && (
                    <div className="bg-coral/20 border-2 border-charcoal rounded-lg p-3">
                      <h3 className="font-bold text-charcoal text-sm mb-2">🛡️ Security Assessment</h3>
                  ...
                  {reviewResults.metadata.overallRoast && (
                    <div className="bg-coral/30 border-2 border-charcoal rounded-lg p-3">
                      <h3 className="font-bold text-charcoal text-sm mb-2">🔥 Overall Roast</h3>
                  ...
                  {reviewResults.metadata.brutalAssessment && (
                    <div className="bg-red-100 border-2 border-charcoal rounded-lg p-3">
                      <h3 className="font-bold text-charcoal text-sm mb-2">💀 Brutal Truth</h3>
                  ...
                  {reviewResults.metadata.positiveAssessment && (
                    <div className="bg-green-100 border-2 border-charcoal rounded-lg p-3">
                      <h3 className="font-bold text-charcoal text-sm mb-2">🌟 Positive Assessment</h3>
              ...
              <div className="space-y-3">
                {reviewResults.suggestions.map((suggestion) => (
                  <div key={suggestion.id} className="bg-cream/50 border-2 border-charcoal rounded-lg p-3">
                    ...
                    <p className="text-charcoal text-sm font-medium mb-2">💡 {suggestion.suggestion}</p>
                    ...
                    <div className="space-y-1 text-xs">
                      ...
                      {suggestion.roastLevel && (
                        <p className="text-charcoal"><strong>🔥 Roast Level:</strong> {suggestion.roastLevel}</p>
                      )}
                      {suggestion.analogiKocak && (
                        <p className="text-charcoal"><strong>😂 Analogi:</strong> {suggestion.analogiKocak}</p>
                      )}
                      {suggestion.consequencesIfIgnored && (
                        <p className="text-charcoal"><strong>⚠️ Consequences:</strong> {suggestion.consequencesIfIgnored}</p>
                      )}
                      {suggestion.learningOpportunity && (
                        <p className="text-charcoal"><strong>📚 Learning:</strong> {suggestion.learningOpportunity}</p>
                      )}
                      {suggestion.confidenceBooster && (
                        <p className="text-charcoal"><strong>✨ You Got This:</strong> {suggestion.confidenceBooster}</p>
                      )}
                    </div>
                    ...
                    {suggestion.codeSnippet.original && (
                      <div className="bg-charcoal rounded p-2 text-xs mt-2">
                        ...
                        {suggestion.codeSnippet.improved && (
                          <>
                            <div className="text-green-400 mt-2 mb-1">✅ After:</div>
              ...
              {reviewResults.metadata && (
                <div className="space-y-3">
                  {reviewResults.metadata.recommendations && reviewResults.metadata.recommendations.length > 0 && (
                    <div className="bg-sky/20 border-2 border-charcoal rounded-lg p-3">
                      <h3 className="font-bold text-charcoal text-sm mb-2">📋 Recommendations</h3>
                  ...
                  {reviewResults.metadata.comedyGold && reviewResults.metadata.comedyGold.length > 0 && (
                    <div className="bg-amber/20 border-2 border-charcoal rounded-lg p-3">
                      <h3 className="font-bold text-charcoal text-sm mb-2">🎭 Comedy Gold</h3>
                      <ul className="list-disc list-inside space-y-1">
                  ...
                  {reviewResults.metadata.encouragements && reviewResults.metadata.encouragements.length > 0 && (
                    <div className="bg-green-100 border-2 border-charcoal rounded-lg p-3">
                      <h3 className="font-bold text-charcoal text-sm mb-2">🌟 Encouragements</h3>
                      <ul className="list-disc list-inside space-y-1">
                  ...
                  {reviewResults.metadata.motivasiSarkastik && (
                    <div className="bg-orange-100 border-2 border-charcoal rounded-lg p-3">
                      <h3 className="font-bold text-charcoal text-sm mb-2">🔥 Motivasi Sarkastik</h3>
